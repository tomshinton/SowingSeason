...
void UUI_SelectionBox::AddInventoryView()
{
	if (InventoryView != nullptr)
	{
		AActor& SelectedActor = Selection->GetActor();
		if (IInventoryInterface* SelectedInventory = SelectedActor.GetInterface<IInventoryInterface>())
		{
			InventoryView->Initialise(*SelectedInventory);
		}
		else
		{
